"""Select small-MLP settings using train-only repeated CV."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("artifacts/results/mlp_cv"),
    )
...
def main() -> None:
    ...
    # classifier-family and hyperparameter
    # selection.
    ...
    # Materialize the folds once so every
    # MLP trial uses exactly the same
    # patients in every split.
    folds = list(
        splitter.split(
            features,
            labels,
    ...
    print("Classifier family: small_mlp")
    ...
    for (
        ...
        for (
            ...
            if len(best_rows) != 1:
                raise RuntimeError(
                    "Expected exactly one history row for the selected best epoch"
                )
        ...
        # completed hyperparameter trial.
